...
from .service import JmfService

logger = logging.getLogger(__name__)

jmf_endpoint = JmfService.blueprint

def ext() -> JmfService:
    ''' convenience for getting the extension-object'''
    return app.specter.ext["jmf"]
...
@jmf_endpoint.route("/")
def index():
	return render_template('jmf/layout.html')
	return render_template('jmf/layout_embedded.html')

@jmf_endpoint.route("/index")
def index_pure():
...
def unlock():
	...
	if request.method == 'GET':
		...
		if listWallets == []:
			return redirect(url_for('jmf_endpoint.create'))
...
@jmf_endpoint.route("/create", methods=['GET', 'POST'])
@app.csrf.exempt
def create():
	if is_backend_down():
		return render_template('error.html')
...
@jmf_endpoint.route("/about")
def about():
	return render_template("about.html")

@jmf_endpoint.route("/get_qr_code")
def get_qr_code():
